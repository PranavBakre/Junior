...
- **Memory** (Junior-internal, `mcp__slack-bot__memory_*`): `memory_recall {query, repo?, kinds?, fact_kinds?, entity_refs?, limit?}` and `memory_add {text, kind?, repo?, tags?}`. `fact_kinds: ["procedure"]` retrieves procedure memories explicitly. `runbook_select` returns a reviewed runbook when one matches and performs procedure recall itself on a miss. Recall/add cadence lives in the core contract; `memory_add` only if the tool is in your `tools:` list.
