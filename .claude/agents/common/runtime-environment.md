...
- **Memory** (Junior-internal, `mcp__slack-bot__memory_*`): `memory_recall {query, repo?, kinds?, entity_refs?, limit?}` - call at task start, before every dispatch, before merge/destructive steps, on an unfamiliar entity, and when surprised. `memory_add {text, kind?, repo?, tags?}` - call once, with a single atomic claim, when corrected or when you learn something durable; only if the tool is in your `tools:` list.
