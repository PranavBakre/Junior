...
export function prepareCodexHome(options: {
  ...
  // app-server processes overlap. Keep the shared base deterministic; each
  // turn supplies authority-bearing values through process-local -c overrides.
  const configPath = resolve(codexHome, "config.toml");
  writeFileAtomic(configPath, buildCodexBaseConfigToml());
...
export function buildCodexBaseConfigToml(): string {
  return [
    "# Generated by Junior. Per-turn authority is supplied as process-local overrides.",
    "",
    "[features]",
    "multi_agent = false",
    "apps = false",
...
export function buildCodexConfigOverrides(options: {
  model: string | null;
  reasoningEffort: string;
  approvalPolicy: CodexApprovalPolicy;
  sandbox: CodexSandbox;
  ...
  if (options.model) overrides.push(`model=${tomlString(options.model)}`);
  overrides.push(
    `model_reasoning_effort=${tomlString(options.reasoningEffort)}`,
    `approval_policy=${tomlString(options.approvalPolicy)}`,
    `sandbox_mode=${tomlString(options.sandbox)}`,
    "features.multi_agent=false",
    "features.apps=false",
  );

  for (const [name, server] of Object.entries(options.mcp ?? {})) {
    const serverPath = `mcp_servers.${tomlBareKey(name)}`;
    if (server.transport) {
      overrides.push(`${serverPath}.transport=${tomlString(server.transport)}`);
    }
    if (server.url) {
      overrides.push(`${serverPath}.url=${tomlString(server.url)}`);
    }
    if (server.command) {
      overrides.push(`${serverPath}.command=${tomlString(server.command)}`);
    }
    if (server.args) {
      overrides.push(
        `${serverPath}.args=[${server.args.map(tomlString).join(", ")}]`,
      );
    }
    for (const [toolName, tool] of Object.entries(server.tools ?? {})) {
      overrides.push(
        `${serverPath}.tools.${tomlBareKey(toolName)}.approval_mode=${tomlString(tool.approval_mode)}`,
      );
    }
  }
  ...
  if (options.trustedProjectPath) {
    overrides.push(
      `projects.${tomlString(options.trustedProjectPath)}.trust_level="trusted"`,
    );
  }
  return overrides;
}
