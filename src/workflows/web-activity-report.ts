...
async function fetchMixpanel(yesterday: string): Promise<MixpanelData> {
  ...
  if (!token) throw new Error(`no Mixpanel token configured for region ${region}`);

  const baseUrl = mixpanelRegionUrl(region).replace("/mcp", "");
  const apiHost = new URL(baseUrl).origin;
