...
1. For an active pipeline assignment, resolve every durable repo ref, provision all managed worktrees, refresh remote refs for reused worktrees, and select the primary cwd from review/workstream affinity. Otherwise resolve `targetRepo` and create its worktree when set.
