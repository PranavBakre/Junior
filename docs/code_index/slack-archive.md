...
| `registerSlackArchiveTools` | `src/mcp/slack-archive-tools.ts` | Signed-turn `slack_archive_search` and `slack_archive_thread` from a public/approved channel (approved DMs included) or an admin-only DM, scoped per result to public/approved channels |
