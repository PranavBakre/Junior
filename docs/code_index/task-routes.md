...
- `src/routes/anchors.test.ts` — real git repo + bare origin: ref resolution, verification reading the ref rather than a dirty checkout, tier 0/1/2/3 outcomes, a symbol MOVED between files, `gone`, `edge-broken`, path-only, tooling notes, pending → activation on merge, path-scoped drift, **a real merge commit carrying an edit** (asserting both the precise first-parent attribution — so it fails on git < 2.31 — and, via `{ diffMerges: false }`, the conservative fallback that would otherwise be untested), **a fetch against a socket that accepts and never answers** (returns inside the 5s bound; pre-fix this hung past 20s with an orphaned helper), unknown sha.
...
- `src/routes/test-fixture.ts` gained `merge(branch, filesInMerge?)` (a real `--no-ff` merge commit carrying edits made during the merge), `rewindOriginRef(sha)` (moves the LOCAL remote-tracking ref back without touching the bare remote — "this box has not fetched in a while"), and `setRemoteUrl(url)` (point `origin` at a host that never answers).
