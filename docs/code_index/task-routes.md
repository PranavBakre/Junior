...
- `src/routes/freshness.ts` — the ref contract that save and verify must agree on.
  - `resolveCanonicalRef(repoPath, defaultBase?, { fetch?, now? })` → `{ repoPath, ref, sha, committedAt }` or **null** (repo absent / ref unresolvable → every step reports `unknown`, never `ok`). Tries `origin/<defaultBase>`, then `refs/remotes/origin/HEAD`, then `origin/main`, `origin/master`. `committedAt` is the ref commit's ISO-8601 date — reported out so an `untouched` answer cannot pass for current when the box is weeks behind.
  - `maybeFetchRef` (private) + `REF_FETCH_MIN_INTERVAL_MS = 10min` / `resetRefFetchThrottle()` — nothing else in this feature fetches, so `{ fetch: true }` runs a best-effort `git fetch origin +refs/heads/<b>:refs/remotes/origin/<b>`, SIGKILLed after 5s, at most once per repo+ref per window (stamped before the call, so an unreachable remote is not re-paid). Remote-tracking refs only; never the working tree.
  ...
  - `routeDrift(ctx, verifiedSha, paths)` — one `git log --name-only --diff-merges=first-parent <verified_sha>..<ref> -- <route paths>` yields both the commit count and the set of paths those commits touched. **`--diff-merges` is load-bearing:** git shows no diff for a merge commit unless asked, so `--name-only` alone printed a bare header and a merge-carried edit was counted but never attributed — the step then answered tier-0 `untouched`/`git-untouched` over a rewritten file, and the clean fetch bumped `verified_sha` past it. On a git without the flag (< 2.31) the log is retried without it; in *both* modes a commit git listed but attributed no path to marks every scoped path changed, which costs a tier-1 fingerprint check and fails safe. Unresolvable sha → `commits: null` (unknown, not zero). `describeDrift` renders `untouched` / `N commits` / `unknown`.
  ...
  - Private `git()` helper: `Bun.spawn` per call, both pipes drained before awaiting exit; an optional timeout SIGKILLs (used only by the network-bound fetch).
...
7. → `{ route_id, resolved, unresolved[], verified_sha, ref, active, identity, overwrote, previous_steps? }`.

### `route_fetch(repo, task, feature?, task_kind?)`
1. Lookup (identity parts slugged on the way in): exact `(repo, feature, task_kind)` first (ignores `active`, so pending/archived routes can be revived), then cosine over `task_desc` filtered to repo/feature with a `SEMANTIC_MIN_COSINE = 0.35` floor (active only). A miss returns `known` — the repo's `(feature, task_kind)` vocabulary, capped at 25.
...
8. → `{ found, route_id, …, verified_sha, ref, ref_committed_at, drift, matched_by, steps[{ ord, note, path, symbol, status, resolved_path?, verified_by, tier, expects_ref? }], confidence, repaired[], active, archived }`, or `{ found: false, repo, reason, known[] }`.
...
- `src/routes/store/memory.test.ts` — the same identity/id/list guarantees for `InMemoryTaskRouteStore`, which is what every tool test runs against; an invariant it does not enforce is one those tests cannot see.
- `src/routes/anchors.test.ts` — real git repo + bare origin: ref resolution, verification reading the ref rather than a dirty checkout, tier 0/1/2/3 outcomes, a symbol MOVED between files, `gone`, `edge-broken`, path-only, tooling notes, pending → activation on merge, path-scoped drift, **a real merge commit carrying an edit** (asserts the precise first-parent attribution, so it fails on git < 2.31 where only the conservative rule is available), unknown sha.
- `src/routes/tools.test.ts` — end-to-end save/fetch/repair/archive/usage against a real repo, the 8-step cap, semantic vs identity lookup, the `unknown` path when the repo is absent, the no-pruning-on-touch guarantee, and one regression case per fixed finding: merge-carried drift, two spellings of one identity, absolute paths in/out of the checkout, `expects_ref` checked at save, auto-fetch of a deliberately-rewound `origin/main`, `known` on a miss, the streak surviving an unreadable repo, and pending activation via tier 2 rather than an import line.
- `src/routes/test-fixture.ts` gained `merge(branch, filesInMerge?)` (a real `--no-ff` merge commit carrying edits made during the merge) and `rewindOriginRef(sha)` (moves the LOCAL remote-tracking ref back without touching the bare remote — "this box has not fetched in a while").
