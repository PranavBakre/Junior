...
- `runner.ts` — `createRunnerInvoke`: production `ConsolidationInvoke`. `runner: "claude" | "opencode"` (default `opencode`) selects the one-shot subprocess (injectable `runText`): `opencode run --model <m> --format json "<prompt>"` (`openCodeRunText` + `buildOpenCodeConsolidationArgs`/`extractOpenCodeAssistantText`, reusing the opencode stream parser) or `claude -p … --output-format json`. The model is ALWAYS pinned — `DEFAULT_OPENCODE_MODEL = opencode-go/deepseek-v4-pro`, `DEFAULT_CLAUDE_MODEL = claude-opus-4-6[1M]` — never the bare CLI default. Output is parsed/validated against `consolidationOutputSchema`; 5-min timeout guard SIGINTs the process tree.
