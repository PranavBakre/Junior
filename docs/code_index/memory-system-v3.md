...
- `consolidate.ts` — `consolidateSession`: reads unconsolidated source records, builds context (existing profiles + nearby claims), calls the injected LLM, persists **episodes → profiles → claims** (cosine proximity-dedup at `DEFAULT_DEDUP_THRESHOLD = 0.92`), stamps records consolidated. Local `cosine` + `fnv1a` (stable 64-bit claim id) helpers.
- `sweep.ts` — `runConsolidationSweep`: the shared orchestration loop (per-thread passes + a final unthreaded sweep, each session isolated so one failure doesn't abort the rest). `summarizeConsolidationSweep` for human output. Used by the CLI, the workflow, and the MCP tool.
...
- `prompt.ts` — `buildConsolidationPrompt`: encodes the HIGH BAR (default output is empty) + shows existing profiles/claims to avoid restating.
