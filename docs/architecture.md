...
2. **Consolidate (offline).** `runConsolidationSweep` first reviews cumulative evidence for active people, repositories, and recurring cross-thread situations, then `consolidateSession` reads unconsolidated thread records and derives **episodes** (affect-tagged log), keyed **profiles** (person/repo/project/situation markdown files under `data/profiles/`, gitignored), and typed atomic **claims** (lessons/facts/preferences/decisions), then stamps the records consolidated so they are processed exactly once. The loopback dashboard exposes these profiles read-only without refreshing their recall timestamps.
