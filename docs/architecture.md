...
2. **Consolidate (offline).** `runConsolidationSweep` first reviews a rolling cross-thread history for each active human to build cumulative person profiles, then `consolidateSession` reads unconsolidated thread records and derives **episodes** (affect-tagged log), keyed **profiles** (person/repo/project/situation markdown files under `data/profiles/`, gitignored), and typed atomic **claims** (lessons/facts/preferences/decisions), then stamps the records consolidated so they are processed exactly once.
