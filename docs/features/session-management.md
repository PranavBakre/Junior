...
- On exit: refetch session (long-running agents go stale; concurrent agents may have written), drain that agent's buffer as one `<buffered-message from="<@USERID>">` block per message (mention resolution renders `from` as `User(Name <@USERID>)`; synthetic internal senders like `mcp-internal` get a bare block with no `from`), or settle to terminal state. Blocks — not flat `User(...):` lines — because a drain can carry multiple authors and multi-line bodies; each block's `from` is authoritative for its own content, and bodies get block delimiters escaped (`escapeBlockDelimiters`) so content can't forge blocks. If the thread was muted mid-turn, discard the buffer.
