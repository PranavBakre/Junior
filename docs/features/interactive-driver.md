...
- **Events from the transcript file, not the TUI.** Claude Code writes a per-session JSONL transcript to `~/.claude/projects/<encoded-cwd>/<sessionId>.jsonl` containing the same event shapes (`user`, `assistant`, `tool_use`, `tool_result`) we already parse in `parser.ts`. The TmuxDriver tails this file and emits events through the same `onEvent(StreamEvent)` callback the rest of the system uses. The TUI render is for humans attaching to debug; the bot never reads it.
- **Turn boundary via Stop hook.** `~/.claude/settings.json` (or per-thread `--mcp-config`-adjacent hook config) registers a `Stop` hook that writes a sentinel to `data/tmux-signals/<sessionName>.stop`. TmuxDriver watches that directory and resolves the in-flight `send()` promise. Hook payload includes the last turn's session_id, which the driver echoes back for sqlite persistence. This replaces "process exited" as the done signal.
