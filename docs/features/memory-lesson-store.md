...
| Abstract principles don't come back from natural situation queries | **0/7** principle hit. But the reviewer traced it: **5 of the 7 targets aren't in the index at all** — they live in `CLAUDE.md` / `MEMORY.md`, never ingested as queryable facts | This is an **ingestion/curation gap, not a proven retrieval gap.** No retrieval mechanism can return a row that was never written. Vectors are *gated behind* first ingesting the principles and then showing a residual that lexical (incl. AND→OR) still misses |
...
- **Phase 1 — curation write-path & ingestion gap:** the `lessons` table; per-session consolidation (hook discipline + capture bar so events stay source-records); lexical dedup as a bootstrap (proximity dedup arrives with Phase 3). **Decide whether the `CLAUDE.md`/`MEMORY.md` principles should be ingested as retrievable facts** — that, not retrieval mechanics, is what the `0/7` actually exposed. *Gate:* memory-health signal-to-noise improves; event flood stops.
