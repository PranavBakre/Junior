...
|---|---|---|
| Real recall traffic is keyword task-fetch, and FTS handles it | **94%** of 365 real logged queries return results | Keep FTS for keyword / exact-identifier lookup |
| Abstract principles are unretrievable from natural situation queries | **0/7** principle hit — the queries return *nothing* | Need a **semantic** channel (vectors) for principles; FTS cannot bridge it |
...
- **The eval harness, the real-log replay, and the principle-retrieval test** — as the gates that decide when each phase ships.
...
- **Phase 0 — done (PR #100):** recall-eval harness (synthetic + real-DB replay), principle-retrieval test, routing-log double-write fix + prune migration.
...
- **Phase 3 — vector channel:** embed lessons (OpenAI, offline batch); parallel FTS ∥ vector retrieval + merge; proximity dedup replaces lexical. *Gate:* principle-retrieval test moves from 0/7 toward passing; p95 within budget.
