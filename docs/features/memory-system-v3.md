...
- **Two retrieval modes decide everything downstream:** **keyed** memory (profiles, fetched by `entity_ref` from context — no vector) and **hybrid** memory (lessons/facts/claims, ranked by vector similarity plus lexical evidence).
