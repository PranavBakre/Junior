...
- Delegated setup requires 2 GiB of free filesystem headroom by default (`WORKTREE_SETUP_MIN_FREE_BYTES` overrides it). Junior drains stdout and stderr concurrently so verbose installers cannot deadlock on full pipes. If the setup script fails after registering a worktree, Junior transactionally removes that worktree and its new branch; the surfaced error is a bounded tail plus a pointer to a complete owner-only (`0600`) transcript under `logs/worktree-setup/`.
