...
- `syncRepo(repoName) → Promise<void>` — refreshes `origin/*` from the configured checkout before a reused pipeline worktree is handed to a runner. Sandboxed providers cannot update the shared Git metadata behind linked worktrees, so this pre-turn fetch belongs to Junior rather than the model process.
