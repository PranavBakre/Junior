...
- Delegated setup requires 2 GiB of free filesystem headroom by default (`WORKTREE_SETUP_MIN_FREE_BYTES` overrides it). If the setup script fails after registering a worktree, Junior transactionally removes that worktree and its new branch; command error output is bounded so an install failure cannot flood Slack or logs.
